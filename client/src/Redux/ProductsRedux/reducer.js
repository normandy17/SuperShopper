    ...
    switch (type) {
        ...
        case FETCH_ALL_PRODUCTS_SUCCESS:
            ...
            for (var i = 0; i < payload.length; i++) {
                var stat = true
                payload[i].image = payload[i].image.split(",")
                payload[i].offer = Math.floor(Number(payload[i].offer))
                payload[i].category = payload[i].category.split(",")

                for (var j = 0; j < cat.length; j++) {
                    if (payload[i].category[0] != cat[j]) stat = true
                    else {
                        stat = false
                        break
                    }
                }
                if (stat == true) cat.push(payload[i].category[0])
            }
            const products2 = [...payload].sort((a, b) => b.offer - a.offer)
